...
function enpassant(moveData) {
 ...
 if ( piece === "♟" || piece === "♙"){
  if(blackpieces.includes(piece) && context.jump.get(`${to.row-1},${to.col}`) === true && whitepieces.includes(boardstate[to.row -1][to.col])){
...
return { islegal: true, state: 'fine', eatenpeices: eatenpeices, eatencolor: 'white', clearedsquare: clearedsquare};
  ...
  else if ( whitepieces.includes(piece) && context.jump.get(`${to.row+1},${to.col}`) === true && blackpieces.includes(boardstate[to.row+1][to.col])){
       let eatenpeices = boardstate[to.row + 1 ][ to.col];
       boardstate[to.row + 1 ][ to.col]=null;
       let clearedsquare =  {row: to.row + 1, col: to.col};
     // White pawn capturing black  
return { islegal: true, state: 'fine', eatenpeices: eatenpeices, eatencolor: 'black', clearedsquare:clearedsquare  };
