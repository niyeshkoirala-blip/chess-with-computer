    ...
    if (nooftimes === 2) {
      ...
      .then(data => {
        ...
        if (state === 'checkmate') {
          ...
          document.getElementById(`${selectedSquare[0].row}-${selectedSquare[0].col}`).textContent = '';
          nooftimes = 0;
          ...
          if(state === 'checkforblackking') {
            const squarewithblackking = Array.from(document.querySelectorAll('.square')).find(div => div.textContent === '♚');
            squarewithblackking.classList.add('check'); turn = color[(color.indexOf(turn) + 1) % color.length];
          }else if(state === 'checkforwhiteking') {
            const squarewithwhiteking = Array.from(document.querySelectorAll('.square')).find(div => div.textContent === '♔');
            squarewithwhiteking.classList.add('check');
          }else {
